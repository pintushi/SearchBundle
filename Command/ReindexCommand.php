...
{
    const COMMAND_NAME = 'pintushi:search:reindex';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($class) {
            $class = $this->getContainer()->get('pintushi_entity.doctrine_helper')->getEntityClass($class);
        }
    ...
    protected function getSearchIndexer($asyncIndexer = false)
    {
        if (true === $asyncIndexer) {
            return $this->getContainer()->get('pintushi_search.async.indexer');
        }

        return $this->getContainer()->get('pintushi_search.search.engine.indexer');
    }
}
